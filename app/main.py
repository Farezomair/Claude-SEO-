...
@app.post("/sites/{site_id}/run-alt")
def run_alt(site_id: int, request: Request, db: Session = Depends(get_db)):
    """Alt-text doer only: write descriptive alt text for images missing it across
    the site's pages (auto-applied + verified live). One doer per page; a page with
    no missing alt is a quick no-op."""
    if not current_user(request):
        return RedirectResponse("/login", status_code=303)
    site = db.query(Site).filter(Site.id == site_id).first()
    if not site:
    ...
    for p in list_elementor_pages(conn)[:30]:
        ...
        if not pid:
            continue
        run = JobRun(site_id=site_id, kind="alttext", status="running",
                     summary=f"Writing alt text on {p.get('title') or pid}…")
        db.add(run)
        db.commit()
        db.refresh(run)
...
@app.post("/sites/{site_id}/run-webp")
def run_webp_now(site_id: int, request: Request, db: Session = Depends(get_db)):
    """WebP doer only: serve images as WebP/AVIF across the site's pages (CDN
    auto=format or convert + rehost), verified live. Clean pages are no-ops."""
    if not current_user(request):
        return RedirectResponse("/login", status_code=303)
    site = db.query(Site).filter(Site.id == site_id).first()
    if not site:
    ...
    for p in list_elementor_pages(conn)[:30]:
        ...
        if not pid:
            continue
        run = JobRun(site_id=site_id, kind="webp", status="running",
                     summary=f"Modernizing images on {p.get('title') or pid}…")
        db.add(run)
        db.commit()
        db.refresh(run)
        start_webp_async(site_id, run.id, conn, pid, p.get("title", ""))
    return RedirectResponse(f"/sites/{site_id}?tab=command", status_code=303)


@app.post("/sites/{site_id}/run-perf")
def run_perf_now(site_id: int, request: Request, db: Session = Depends(get_db)):
    """Performance doer only: lazy-load offscreen images across the site's pages
    (hero kept eager for LCP). Real CWV improvement; field data reflects over ~4 weeks."""
    if not current_user(request):
        return RedirectResponse("/login", status_code=303)
    site = db.query(Site).filter(Site.id == site_id).first()
    if not site:
    ...
    for p in list_elementor_pages(conn)[:30]:
        ...
        if not pid:
            continue
        run = JobRun(site_id=site_id, kind="perf", status="running",
                     summary=f"Optimizing {p.get('title') or pid}…")
        db.add(run)
        db.commit()
        db.refresh(run)
        start_perf_async(site_id, run.id, conn, pid, p.get("title", ""))
    return RedirectResponse(f"/sites/{site_id}?tab=command", status_code=303)
...
def approve(approval_id: int, request: Request, publish: int = Form(0),
    ...
    # One implementation for buttons AND the Fix Chat (approval_actions).
    from .approval_actions import apply_approval
    ok, notice = apply_approval(db, appr, bool(publish))
    return RedirectResponse(_with_notice(ret, notice), status_code=303)
...
def reject(approval_id: int, request: Request, return_to: str = Form(""),
    ...
    if not appr or appr.status != "pending":
        return RedirectResponse(ret, status_code=303)
    from .approval_actions import reject_approval
