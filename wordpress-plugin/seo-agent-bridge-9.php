<?php
/**
 * Plugin Name: SEO Agent Bridge 9
 * Description: Lets the SEO Agent (Ascend) read/update Yoast meta, Additional CSS, the Elementor HTML widget, and the _meridian_body field — and, new in v1.4, manage TECHNICAL SEO at the server level: send security response headers, serve /llms.txt (HTTP 200), 301-REDIRECT dead/old URLs, inject <head> tags (canonical / OG / viewport / favicon), extend or override robots.txt, NOINDEX thin archive pages (Yoast tag/category/author), and enforce the SEO title on the rendered page when the theme overrides it. All toggleable via REST and fully reversible. Adds nothing visible to the front end on its own.
 * Version: 1.7.0
...
add_action('template_redirect', function () {
    if (is_admin() || is_feed() || !get_option('seo_agent_title_override')) { return; }
    $t = seo_agent_resolved_title();
    if ($t === '') { return; }
    ob_start(function ($html) use ($t) {
        return preg_replace('#<title[^>]*>.*?</title>#si',
